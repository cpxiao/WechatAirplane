package com.cpxiao.wechatairplane.mode.common;

import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.graphics.RectF;
import android.util.Log;

import com.cpxiao.wechatairplane.views.GameView;

/**
 * 走直线的Sprite类，其位置只能直上直下
 */
public class AutoSprite extends Sprite {
    //每帧移动的像素数,以向下为正
    private int speed = 2;
    //偏移角度，以水平向右为0，顺时针方向为正。
    private int angle = 90;

    public AutoSprite(Bitmap bitmap) {
        super(bitmap);
    }

    public void setSpeedAndAngle(int speed, int angle) {
        this.speed = speed;
        this.angle = angle;
    }

    public void setSpeed(int speed) {
        this.speed = speed;
    }

    public float getSpeed() {
        return speed;
    }

    public void setAngle(int angle) {
        this.angle = angle;
    }

    public int getAngle() {
        return angle;
    }

    @Override
    protected void beforeDraw(Canvas canvas, Paint paint, GameView gameView) {
        if (!isDestroyed()) {
            //在y轴方向移动speed像素
            //            move(0, speed * gameView.getDensity());
            float moveX = (float) (Math.cos(Math.PI * angle / 180) * speed * gameView.getDensity());
            float moveY = (float) (Math.sin(Math.PI * angle / 180) * speed * gameView.getDensity());
            if (DEBUG) {
                Log.d(TAG, "beforeDraw: angle = " + angle + ", moveX = " + moveX + ", moveY = " + moveY);
            }
            move(moveX, moveY);
        }
    }

    protected void afterDraw(Canvas canvas, Paint paint, GameView gameView) {
        if (!isDestroyed()) {
            //检查Sprite是否超出了Canvas的范围，如果超出，则销毁Sprite
            RectF canvasRecF = new RectF(0, 0, canvas.getWidth(), canvas.getHeight());
            RectF spriteRecF = getRectF();
            if (!RectF.intersects(canvasRecF, spriteRecF)) {
                destroy();
            }
        }
    }
}