package com.cpxiao.wechatairplane.mode;
...
public class CombatAircraft extends Sprite {
    ...
    private static final int maxDoubleTime = 140;//使用双子弹最多绘制的次数

    //被撞击后闪烁相关
    private long beginFlushFrame = 0;//要在第beginFlushFrame帧开始闪烁战斗机
    private int flushTime = 0;//已经闪烁的次数
    private static final int flashFrequency = 16;//在闪烁的时候，每隔16帧转变战斗机的可见性
    private static final int maxFlashTime = 10;//最大闪烁次数，之后销毁
    ...
    protected void afterDraw(Canvas canvas, Paint paint, GameView gameView) {
        ...
        if (beginFlushFrame > 0) {
            ...
            if (frame >= beginFlushFrame) {
                if ((frame - beginFlushFrame) % flashFrequency == 0) {
                    boolean visible = getVisibility();
                    setVisibility(!visible);
                    flushTime++;
                    if (flushTime >= maxFlashTime) {
